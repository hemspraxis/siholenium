...
public class CommonCRUDS extends BrowserBehaviour {
	...
	@Deprecated
	public boolean agregarRegistroSinClave(WebElement descTxt, WebElement guardarBtn, WebElement altaExitoMsg, String clave, String descripcion) throws Exception {
		boolean ifIdExists = true;
		boolean success = false;
		String responseMessage = null;
		while(ifIdExists){
			responseMessage = getMessageFromAgregarRegistroSinClave(descTxt, guardarBtn, altaExitoMsg, descripcion);
			if (responseMessage.equals("Registro almacenado correctamente.")){
				ifIdExists = !(success = true);
				return success;
			} else if (responseMessage.equals("El Id. ya existe, favor de validar.")) {
	...
	@Deprecated
	public boolean editarRegistroSinClave(WebElement descTxt, WebElement guardarEdicionBtn, WebElement altaExitoMsg, WebElement tbody, WebElement nextPage, String descripcionActual, String descripcionNueva) throws Exception {
		boolean success = false;
		boolean ifIdExists = true;
		String responseMessage = null;
		while (ifIdExists){
			responseMessage = getMessageFromEditarRegistroSinClave(descTxt, guardarEdicionBtn, altaExitoMsg, tbody, nextPage, descripcionActual, descripcionNueva);
			if (responseMessage.equals("Registro actualizado correctamente.")) {
				ifIdExists = !(success = true);
			} else {
				System.out.println("Respuesta no exitosa para editar linea tecnologica");
	...
	public boolean borrarRegistroSinClave(WebElement eliminarBtn, WebElement successMsg, 
			WebElement tbody, WebElement nextPage, String descripcion){
		boolean success = false;
		WebElement check = getCheckBoxSinClave(tbody, nextPage, descripcion);
		if (check != null) {
			waitForWebElementDisplayed(check);
			safeClick(check);
			safeClick(eliminarBtn);
	...
	public String getMessageFromAgregarRegistroSinClave(WebElement descTxt, WebElement guardarBtn, WebElement altaExitoMsg,String descripcion) throws Exception {
		String resultMessage = null;
		if (descripcion != null) {
			Thread.sleep(1000);
			safeSendKeys(descTxt, descripcion);
	...
	public String getMessageFromEditarRegistroSinClave(WebElement descTxt, WebElement guardarEdicionBtn, WebElement altaExitoMsg, WebElement tbody, WebElement nextPage, String descripcionActual, String descripcionNueva) throws Exception {
		String resultMessage = null;
		WebElement openEditarButton = getButtonEditarSinClave(tbody, nextPage, descripcionActual);
		waitForWebElementClickable(openEditarButton);
		if (openEditarButton != null) {
			safeClick(openEditarButton);
			try {
				safeSendKeys(descTxt, descripcionNueva);
				safeClick(guardarEdicionBtn);
				waitForWebElementDisplayed(altaExitoMsg);
				resultMessage = safeGetLabel(altaExitoMsg);
				if (resultMessage == null || resultMessage.matches("([^\\w]|$)")){
	...
	private WebElement getButtonEditar(WebElement tbody, WebElement nextPage, String clave) {
		...
		for (WebElement tr : tbody.findElements(By.tagName("tr"))) {
			...
			if (tds.get(1).getText().toUpperCase().trim().equals(clave.toUpperCase().trim())) {
	...
	private WebElement getButtonEditarSinClave(WebElement tbody, WebElement nextPage, String descripcionActual) {
		try {
			Thread.sleep(1500);
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		for (WebElement tr : tbody.findElements(By.tagName("tr"))) {
			waitForWebElementDisplayed(tr);
			List<WebElement> tds = tr.findElements(By.tagName("td"));
			if (tds.get(2).getText().toUpperCase().trim().equals(descripcionActual.toUpperCase().trim())) {
				System.out.println("__________found!__________");
				return tds.get(4).findElement(By.tagName("button"));
			} else {
				System.out.println("not found yet");
			}
		}
		...
		if (nextPage.isEnabled()) {
			safeClick(nextPage);
			return getButtonEditarSinClave(tbody, nextPage, descripcionActual);
		} else {
			return null;
		}
	}
	...
	private WebElement getCheckBoxSinClave(WebElement tbody, WebElement nextPage, String descripcion) {
		int count = 0;
		System.out.println("this is the occurrence: " + count++);
		try {
			Thread.sleep(1500);
		...
		for (WebElement tr : tbody.findElements(By.tagName("tr"))) {
			...
			List<WebElement> tds = tr.findElements(By.tagName("td"));
			System.out.println("tds : " + tds.size());
			System.out.println(tds.get(2).getText().toUpperCase().trim() + " is it equals to: " + descripcion.toUpperCase().trim());
			if (tds.get(2).getText().toUpperCase().trim().equals(descripcion.toUpperCase().trim())) {
				System.out.println("__________found!__________");
				return tds.get(0).findElement(By.tagName("span"));
			} else {
				System.out.println("checkbox not found yet");
			}
		}
		if (nextPage.isEnabled()) {
			System.out.println("another cycle");
			safeClick(nextPage);
			System.out.println("after clicking netxpage");
			return getCheckBoxSinClave(tbody, nextPage, descripcion);
		} else {
			return null;
		}
	}
